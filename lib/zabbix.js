...
const getGroupId = async (server, token, group) => {
  try {
    ...
    if (zabbixResponse.result === undefined) {
      const result = {
        data: null,
        message: `Cannot access to zabbix: check the token is correct and configured in zabbix`,
      };
      return result;
    }
    if (zabbixResponse.result[0] === undefined) {
      const result = {
        data: null,
        message: `Group ${group} does not exist: correct group name or make sure it is configured in zabbix server`,
      };
      return result;
    }
    ...
    const result = {
      data: groupId,
      message: `Zabbix group ${group} found with id ${groupId}`,
    };
    return result;
  } catch (error) {
