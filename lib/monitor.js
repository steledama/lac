...
const monitorDevices = async () => {
  try {
    ...
    devices.forEach(async (device) => {
      ...
      itemsValues.forEach(async (item) => {
        const itemSendResult = await sendItemToZabbix(
          conf.server,
          device.host,
          item.oid,
          item.value
    ...
    const result = {
      variant: 'success',
      text: 'SUCCESS: Devices monitored and results sent to zabbix with positive response',
    };
    return result;
  } catch (error) {
    console.log(error);
